...
public class WorldSimTests
{
    ...
    public void PlaceMachine_RejectsOccupiedRegion()
    {
        var sim = TestWorld.BuildSim();
        sim.ReceiveCommand(new PlaceMachineAt("chest", new Vec2I(2, 3)));
        sim.ReceiveCommand(new PlaceMachineAt("chest", new Vec2I(3, 3))); // overlaps the first one
        sim.Tick();
    ...
    {
        var sim = TestWorld.BuildSim();
        sim.ReceiveCommand(new PlaceEntityAt("player", new Vec2(2.5f, 3.5f)));
        sim.ReceiveCommand(new PlaceMachineAt("chest", new Vec2I(2, 3)));
        sim.Tick();
    ...
    public void MoveEntity_IsBlockedByMachine()
    {
        ...
        for (var i = 0; i < 100; i++)
        {
            sim.ReceiveCommand(new EntityStepById(playerId, new Vec2(0.25f, 0))); // step right
            sim.Tick();
        }
    ...
    public void MoveEntity_SlidesAlongBlockedAxis()
    {
        ...
        for (var i = 0; i < 4; i++)
        {
            sim.ReceiveCommand(new EntityStepById(playerId, new Vec2(0.25f, 0.25f)));
            sim.Tick();
        }
    ...
    public void PlaceMachine_OnBehalfOfPlayerRespectsReach()
    {
        ...
        sim.ReceiveCommand(new PlaceMachineAt("chest", new Vec2I(40, 40), actorId: playerId));
    ...
    public void RemoveMachine_FreesTheRegion()
    {
        ...
        sim.ReceiveCommand(new PlaceMachineAt("chest", new Vec2I(2, 3)));
        sim.Tick();
        sim.ReceiveCommand(new RemoveMachineAt(new Vec2I(3, 3))); // second tile of the chest
        sim.Tick();
    ...
    public void SetGround_ReplacesTileAndRaisesEvent()
    {
        ...
        sim.ReceiveCommand(new SetGroundAt(newKind, new Vec2I(100, 100)));
        sim.Tick();
    ...
    public void MachinesInFarChunksKeepTicking()
    {
        ...
        sim.ReceiveCommand(new PlaceMachineAt("chest", far + new Vec2I(0, 0)));
        sim.ReceiveCommand(new PlaceMachineAt("manipulator", far + new Vec2I(2, 0)));
        sim.ReceiveCommand(new PlaceMachineAt("chest", far + new Vec2I(2, 1)));
        sim.ReceiveCommand(new InsertItemAt("iron_bar", 8, far));
    ...
    public void MachineOnChunkBorder_IsListedInBothChunks()
    {
        ...
        sim.ReceiveCommand(new PlaceMachineAt("chest", new Vec2I(TestWorld.ChunkSize - 1, 0)));
        sim.Tick();
        ...
        sim.ReceiveCommand(new RemoveMachineAt(new Vec2I(TestWorld.ChunkSize, 0))); // its second tile
        sim.Tick();
