...
public class DepositTests
{
    ...
    private static List<Vec2I> FindDeposits(WorldSim sim, int radius)
    {
        ...
        for (var y = -radius; y < radius; y++)
        {
            ...
            if (sim.ChunkManager.GetGroundLiteAt(coord).Kind == "iron_deposit")
                deposits.Add(coord);
        }
        return deposits;
    }
    ...
    [Fact]
    public void Generator_ProducesDepositVeins()
    {
        var sim = TestWorld.BuildSim();
        var deposits = FindDeposits(sim, 60);
